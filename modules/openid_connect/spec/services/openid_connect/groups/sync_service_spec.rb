...
RSpec.describe OpenIDConnect::Groups::SyncService do
  ...
  it "links the OIDC group to the local group" do
    call_service
    expect(Group.first.oidc_group_links.where(oidc_name: "oidc_group_a", auth_provider: provider)).to be_exist
  end
  ...
  context "when a group already exists" do
    context "and it has the same name" do
      ...
      it "links the OIDC group to the local group" do
        call_service
        expect(existing_group.oidc_group_links.where(oidc_name: "oidc_group_a", auth_provider: provider)).to be_exist
      end
    ...
    context "and it is linked to the OIDC group" do
      ...
      let!(:group_link) { create(:oidc_group_link, group: existing_group, auth_provider: provider, oidc_name: "oidc_group_a") }
  ...
  describe "group prefix matching" do
    ...
    it "links the OIDC group to the local group with correct names on both ends" do
      ...
      expect(OpenIDConnect::GroupLink.find_by(oidc_name: "/abc/def/ghi")&.group&.name).to eq("def/ghi")
    end
  ...
  describe "group regexp matching" do
    ...
    it "links the OIDC group to the local group with correct names on both ends" do
      ...
      expect(OpenIDConnect::GroupLink.find_by(oidc_name: "/abc/def/ghi")&.group&.name).to eq("de")
    end
