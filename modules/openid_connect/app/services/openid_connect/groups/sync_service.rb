  ...
  module Groups
    class SyncService
      ...
      def call(groups_claim:)
        ...
        groups_claim.each do |oidc_name|
          group = process_oidc_group(oidc_name)
      ...
      def process_oidc_group(oidc_name)
        local_name = filter_and_name_group(oidc_name)
        return nil if local_name.nil?

        group = find_group(oidc_name, local_name)
      ...
      def find_group(oidc_name, local_name)
        group_link = GroupLink.find_or_initialize_by(auth_provider: authentication_provider, oidc_name:)
        if group_link.group.nil?
          group_link.group = ::Group.find_or_create_by!(name: local_name)
          group_link.save!
        end
