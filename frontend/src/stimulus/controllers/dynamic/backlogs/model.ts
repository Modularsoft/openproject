...
interface ModelInstance {
  ...
  prepareInputFromFactory(input:JQuery, fieldId:string, fieldName:string, fieldOrder:number, maxTabIndex:number, fieldEditable?:string):JQuery;
  ...
  refresh(obj:ModelInstance):void;
  refreshed():void;
...
(window as any).RB.Model = (function ($:JQueryStatic) {
  return (window as any).RB.Object.create({

    initialize(this:ModelInstance, el:HTMLElement):void {
      this.$ = $(el);
      this.el = el;
    },

    afterCreate(this:ModelInstance, data:any, textStatus:string, xhr:JQuery.jqXHR):void {
      // Do nothing. Child objects may optionally override this
    },

    afterSave(this:ModelInstance, data:any, textStatus:string, xhr:JQuery.jqXHR):void {
      let isNew:boolean;
      let result:ModelInstance;

      isNew = this.isNew();
      result = (window as any).RB.Factory.initialize((window as any).RB.Model, data);
    ...
    afterUpdate(this:ModelInstance, data:any, textStatus:string, xhr:JQuery.jqXHR):void {
      // Do nothing. Child objects may optionally override this
    },

    beforeSave(this:ModelInstance):void {
      // Do nothing. Child objects may or may not override this method
    },

    cancelEdit(this:ModelInstance):void {
      this.endEdit();
      if (this.isNew()) {
        this.$.hide('blind');
      }
    ...
    close(this:ModelInstance):void {
      this.$.addClass('closed');
    },

    copyFromDialog(this:ModelInstance):void {
      let editors:JQuery;
    ...
    displayEditor(this:ModelInstance, editor:JQuery):void {
      const self = this;
      ...
      editor.dialog({
        ...
        close(e:Event, ui:any):void {
          if ((e as any).which === 1 || (e as any).which === 27) {
            self.cancelEdit();
          }
        },
    ...
    edit(this:ModelInstance):JQuery {
          ...
          if (fieldType === 'select') {
            ...
            if (fieldName === 'status_id') {
              ...
              input.change(function (this:HTMLElement):void {
                ...
                self.replaceStatusForNewType(input, newInput, $(this).parent().find('.status_id').val() as string || '', editor);
              });
            } else {
              input = $(`#${fieldName}_options`).clone(true);
            }
    ...
    findFactory(this:ModelInstance, typeId:string, statusId:string, fieldName:string):JQuery {
      // Find a factory
    ...
    prepareInputFromFactory(this:ModelInstance, input:JQuery, fieldId:string, fieldName:string, fieldOrder:number, maxTabIndex:number, fieldEditable:string = 'true'):JQuery {
    ...
    replaceStatusForNewType(this:ModelInstance, input:JQuery, newInput:JQuery, statusId:string, editor:JQuery):void {
      const self = this;
    ...
    editDialogTitle(this:ModelInstance):string {
      return `Edit ${this.getType()}`;
    },

    editorDisplayed(this:ModelInstance, editor:JQuery):void {
      // Do nothing. Child objects may override this.
    },

    endEdit(this:ModelInstance):void {
      this.$.removeClass('editing');
    },

    error(this:ModelInstance, xhr:JQuery.jqXHR, textStatus:string, error:string):void {
      this.markError();
      (window as any).RB.Dialog.msg($(xhr.responseText).find('.errors').html());
      this.processError(xhr, textStatus, error);
    },

    getEditor(this:ModelInstance):JQuery {
      let editorId:string;
      let editor:JQuery;
    ...
    handleSelect(this:HTMLElement, e:any):void {
      const j = $(this);
      const self = j.data('this') as ModelInstance;
    ...
    isClosed(this:ModelInstance):boolean {
      return this.$.hasClass('closed');
    },

    isNew(this:ModelInstance):boolean {
      return this.getID() === '';
    },

    markError(this:ModelInstance):void {
      this.$.addClass('error icon icon-bug');
    },

    markIfClosed(this:ModelInstance):void {
      throw new Error('Child objects must override markIfClosed()');
    },

    markSaving(this:ModelInstance):void {
      this.$.addClass('ajax-indicator');
    },

    // Override this method to change the dialog title
    newDialogTitle(this:ModelInstance):string {
      return `New ${this.getType()}`;
    },

    open(this:ModelInstance):void {
      this.$.removeClass('closed');
    },

    processError(this:ModelInstance, x:any, t:string, e:string):void {
      // Override as needed
    },

    refresh(this:ModelInstance, obj:ModelInstance):void {
      this.$.html(obj.$.html());
    ...
    refreshed(this:ModelInstance):void {
      // Override as needed
    },

    saveDirectives(this:ModelInstance):SaveDirectives {
      throw new Error('Child object must implement saveDirectives()');
    },

    saveEdits(this:ModelInstance):void {
      const j = this.$;
      const self = this;
      ...
      editors.each(function (this:HTMLElement, index:number):void {
        ...
        if (type && type.match(/select/)) {
          ...
          if (!(editor.val() === '' && fieldName === 'status_id')) {
            j.children(`div.${fieldName}`).children('.v').text(editor.val() as string || '');
          }
    ...
    unmarkError(this:ModelInstance):void {
      this.$.removeClass('error icon icon-bug');
    },

    unmarkSaving(this:ModelInstance):void {
